		...
		<div class="profile-head">
			<div class="col-md-12 col-lg-4 col-xl-3">
				<div class="image-content-center user-pro">
					<div class="preview">
						<img src="<?=get_image_url('student', $student['photo']);?>">
					</div>
				</div>
			</div>
				...
				<div id="profile" class="accordion-body collapse <?=($this->session->flashdata('profile_tab') == 1 ? 'in' : ''); ?>">
					...
					<div class="panel-body">
							...
							<div class="col-md-<?=$widget; ?> mb-sm">
								<div class="form-group">
									...
									<?php
										...
										onchange='getSectionByClass(this.value,0); getRollByClass(this.value);' data-plugin-selectTwo data-width='100%' data-minimum-results-for-search='Infinity' ");
									?>
						...
						<div class="row">
							<div class="col-md-4 mb-sm">
								<div class="form-group">
									<label class="control-label"><?=translate('PEN_number')?></label>
									<input type="text" class="form-control" name="pen" value="<?=set_value('pen', $student['pen'])?>" />
								</div>
							</div>
							<div class="col-md-4 mb-sm">
								<div class="form-group">
						...
						<div class="row" id="customFields">
							<?=render_custom_Fields('student', $student['branch_id'], $student['id']); ?>
						</div>
						...
						<div class="row">
							<div class="col-md-12 mb-sm">
								<div class="form-group">
									...
									<input type="hidden" name="old_user_photo" value="<?=$student['photo']; ?>" />
								</div>
							...
							<table class="table table-bordered table-condensed table-hover mb-none tbr-top">
								...
								<tfoot>
									<tr class="text-dark">
										...
										<th></th>
										<th></th>
										<th><?=$currency_symbol . number_format($total_amount, 2, '.', ''); ?></th>
										<th><?=$currency_symbol . number_format($total_discount, 2, '.', ''); ?></th>
										<th><?=$currency_symbol . number_format($total_fine, 2, '.', ''); ?></th>
										<th><?=$currency_symbol . number_format($total_paid, 2, '.', ''); ?></th>
										<th><?=$currency_symbol . number_format($total_balance, 2, '.', ''); ?></th>
									</tr>
								</tfoot>
							</table>
						</div>
							...
							<table class="table table-bordered table-hover table-condensed mb-none">
								...
								<tbody>
									...
									$book_result = $this->db->get('book_issues')->result_array();
										...
										<tr>
											<td><?=$count++;?></td>
											<td><?=get_type_name_by_id('book', $book['book_id'], 'title');?></td>
											<td><?=_d($book['date_of_issue']);?></td>
											<td><?=_d($book['date_of_expiry']);?></td>
											<td>
												<?php
										...
										<table class="table table-condensed table-bordered mt-sm">
											<thead>
												...
												foreach ($markDistribution as $id) {
													?>
													<th><?=get_type_name_by_id('exam_mark_distribution',$id)  ?></th>
												<?php } ?>
							...
							<table class="table table-striped table-bordered table-condensed mb-none">
								<tbody>
									<tr>
										...
										<td><?=$getParent['relation']?></td>
									</tr>
									<tr>
										<th><?=translate('occupation')?></th>
										<td><?=$getParent['occupation']?></td>
										<th><?=translate('income')?></th>
										<td><?=$global_config['currency_symbol'] . $getParent['income']?></td>
									</tr>
									<tr>
										<th><?=translate('education')?></th>
										<td><?=$getParent['education']?></td>
										<th><?=translate('city')?></th>
										<td><?=$getParent['city']?></td>
									</tr>
									<tr>
										<th><?=translate('state')?></th>
										<td><?=$getParent['state']?></td>
										<th><?=translate('mobile_no')?></th>
										<td><?=$getParent['mobileno']?></td>
									</tr>
									<tr>
										<th><?=translate('email')?></th>
										<td colspan="3"><?=$getParent['email']?></td>
									</tr>
									<tr class="quick-address">
										<th><?=translate('address')?></th>
										<td colspan="3" height="80px;"><?=$getParent['address']?></td>
									</tr>
								</tbody>
							</table>
						</div>
                    ...
                    <div class="panel-body">
                        <div class="text-right mb-sm">
                            <a href="javascript:void(0);" onclick="mfp_modal('#addStaffDocuments')" class="btn btn-circle btn-default mb-sm">
                                <i class="fas fa-plus-circle"></i> <?=translate('add') . " " . translate('document'); ?>
                            </a>
                        </div>
                        <div class="table-responsive mb-md">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th><?=translate('document') . " " . translate('type'); ?></th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="min-w-c">
                                        <a href="<?=base_url('student/documents_download?file=' . $row->enc_name); ?>" class="btn btn-default btn-circle icon" data-toggle="tooltip" data-original-title="<?=translate('download')?>">
                                            <i class="fas fa-cloud-download-alt"></i>
                                        </a>
                                        <a href="javascript:void(0);" class="btn btn-circle icon btn-default" onclick="editDocument('<?=$row->id?>', 'student')">
                                            <i class="fas fa-pen-nib"></i>
                                        </a>
                                        <?=btn_delete('student/document_delete/' . $row->id); ?>
                                    </td>
                                </tr>
                                <?php
                                    endforeach;
...
<div id="authentication_modal" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
	<section class="panel">
		...
		<?=form_open('student/change_password', array('class' => 'frm-submit')); ?>
        <div class="panel-body">
...
<div id="addStaffDocuments" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
    <section class="panel">
        <div class="panel-heading">
            <h4 class="panel-title"><i class="fas fa-plus-circle"></i> <?=translate('add') . " " . translate('document'); ?></h4>
        </div>
        <?=form_open_multipart('student/document_create', array('class' => 'form-horizontal frm-submit-data')); ?>
            <div class="panel-body">
                <input type="hidden" name="patient_id" value="<?=$student['id']; ?>">
                ...
                <div class="form-group">
                    <label class="col-md-3 control-label"><?=translate('document') . " " . translate('type'); ?> <span class="required">*</span></label>
                    <div class="col-md-9">
                ...
                <div class="form-group">
                    <label class="col-md-3 control-label"><?=translate('document') . " " . translate('file'); ?> <span class="required">*</span></label>
                    <div class="col-md-9">
...
<div id="editDocModal" class="zoom-anim-dialog modal-block modal-block-primary mfp-hide">
    <section class="panel">
        <div class="panel-heading">
            <h4 class="panel-title"><i class="far fa-edit"></i> <?=translate('edit') . " " . translate('document'); ?></h4>
        </div>
        <?=form_open_multipart('student/document_update', array('class' => 'form-horizontal frm-submit-data')); ?>
            <div class="panel-body">
