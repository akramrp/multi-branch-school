...
class Student_model extends MY_Model
{
    ...
    public function csvImport($row, $classID, $sectionID, $branchID)
    {
        ...
        if (is_array($getParent) && count($getParent)) {
            ...
            // echo '<pre>getParent:'; print_r($arrayParent); die;
            $this->db->insert('parent', $arrayParent);
            $parentID = $this->db->insert_id();
            $parent_credential = array(
    ...
    // get roll based on the class
    public function generateRollNumber($classID) {
        if (!empty($classID)) {
            $count = $this->db->get_where('enroll', ['class_id' => $classID])->num_rows();
            $rollNo = $count ? $count + 1 : '1';
        } else {
            $rollNo = '1';
        }
        return $rollNo;
